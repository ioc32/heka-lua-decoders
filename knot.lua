...
local msg_type      = read_config("type") or "knot-log"
...
local ts = l.Cg(l.Ct(dt.date_fullyear * "-" * dt.date_month * "-" * date_mday_sp * "T" * dt.rfc3339_partial_time) / dt.time_to_ns, "time")
local severity = l.Cg(syslog_severity_text, "severity")
local message = l.Cg(l.P(1)^0, "message")
--local message = l.Cg(l.P(-1), "message")

local knot_grammar = l.Ct(ts * sp * severity * ":" * sp * message)
...
function process_message ()
    ...
    local fields = knot_grammar:match(log)
